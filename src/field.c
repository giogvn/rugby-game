...
{
  if (field == NULL)
    return (dimension_t)NULL_DIMENSION;
...
char *string_field(Field field)
{
  ...
  for (size_t i = 0; i < field->dimension.height; i++)
  {
    for (size_t j = 0; j < field->dimension.width; j++)
    {
      field_string[i * field->dimension.width + j] = get_item_symbol(field->grid[i][j]);
    }
  }
  return field_string;
}
...
void move_item_in_field(Field field, Item item, direction_t direction)
{
  ...
  if (field->grid[new_position.i][new_position.j] != NULL)
    return;
